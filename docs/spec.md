                                                                                                                                             ...
                                                                                                                                             timestamp - LAG(timestamp) OVER (
                                                                                                                                                   PARTITION BY item_name ORDER BY timestamp
                                                                                                                                                         ...
                                                                                                                                                         FROM shopping_logs
                                                                                                                                                           ...
                                                                                                                                                           #### 6.7 Household Calendar *(new)*
                                                                                                                                                           Native calendar, aggregating read-only external feeds and AI/user-created events into one Postgres table — no Notion mirror (unlike §6.4), following the §6.5 precedent that a domain gets a native in-app view instead of a Notion surface when one isn't needed. Phase 1 scope: Cozi + Outlook ICS ingestion (read-only) plus native event creation. Google Calendar's 2-way OAuth sync is deliberately deferred to a later phase — heavier lift (refresh-token flow, incremental `syncToken`, conflict handling on genuinely bidirectional writes) that doesn't need to block read-only aggregation shipping first.

                                                                                                                                                           - **Schema (`calendar_events`):** `source` enum (`'cozi' | 'outlook' | 'native'` — `'google'` added when phase 2 lands), `external_id` (feed UID, unique per source), `user_id` (required, RLS-scoped exactly like `list_items` — see below), `title`/`description`/`location`/`start_time`/`end_time`/`all_day`, `assigned_members text[]` (informational household-member tags, same role as `lists.tags` — never a security boundary; `user_id` is). `color_code`/`is_read_only` are *not* stored columns — both are pure functions of `source`, computed at read time (`plugins/calendar/src/sourceMeta.ts`), so there's exactly one place that knows what a source means, per the canonical-record principle.
                                                                                                                                                           - **Ownership for shared feeds:** Cozi is one shared family `.ics` URL, not per-member — same shape as Notion's single-workspace sync. Attributed to one owning user via `BIGBRAIN_CALENDAR_OWNER_USER_ID`, mirroring `BIGBRAIN_NOTION_OWNER_USER_ID` (§6.4). Outlook (work) and native events are already tied to whichever user's tools created them.
                                                                                                                                                           ...
                                                                                                                                                           - **Privacy masking:** `MASK_WORK_CALENDAR` env flag; when set, the Outlook parser (a pure function, `plugins/calendar/src/parseWorkEvent.ts`) replaces `title`/`description`/`location` with a fixed placeholder before the row ever reaches Postgres. Config-driven, not LLM-judged — this is redaction, not reasoning, so it doesn't touch Principle 2.
                                                                                                                                                           ...
                                                                                                                                                           - **Tools (`plugins/calendar`):** `get_calendar_schedule(startDate, endDate, sources?)` and `create_calendar_event(title, start_time, end_time, description?, assigned_members?)` (sets `source = 'native'`). Update/delete on native events deferred until real use shows they're needed, same as this project's general bias toward shipping the minimum and expanding on live friction rather than anticipated need.
