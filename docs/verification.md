...
| `orchestrator/scripts/verify-location-presence-scraper.mjs` | The post-cleanup scraper (docs/plans/vistalyze_integration/segway.md §4): the pure two-line header parse, location/scene/character resolve-or-create, scene_presence replace, the §2.6 eligibility filter, and the fail-open contract (fake pool) | Real LLM header quality — whether Stage 1's cleanup pass actually emits the block |
