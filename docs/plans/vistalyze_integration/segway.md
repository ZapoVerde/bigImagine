...
3. **No match**: create a new `locations` row — `status = 'transient'`, `anchor_chat_id` = this chat, `anchor_swipe_id` = the current active swipe, `visual_description` seeded from the extracted name (`definition` null), `environment` from the extracted time/date, `seed` = the shared fixed image-gen seed (endpoint.md §1). **Same-place carry**: before minting, the most recent same-named row rendered in *this* chat (`anchor_chat_id`-scoped, so another chat's row of the same name never leaks in) has its `seed`/`image_url`/`image_rendered_input`/`image_render_hash` cloned onto the new row — and, when that prior row was actually described (its `visual_description` differs from its name), its `visual_description`/`definition` ride along too. Why: the rerun of the turn that anchored a row supersedes its swipe, making the row ineligible for the next scrape of the same room — without the carry that would mint a blank row and re-render a pixel-identical background (the room image is canonical: fixed seed + a template with no time-of-day, endpoint.md §4.2); with it, the follow-up generation pass (endpoint.md §5.1 step 2) is a hash hit — zero provider cost, and the visible background never changes for the same room. The description must ride along with the fingerprint because the render hash is computed over the *described* prompt: a described prior's hash would mismatch a name-seeded fresh row and force a name-only re-render (worse image + wasted gen). A genuinely new place (no prior image) mints blank and renders as before — with the room describer (describer.md) enriching the name-seeded description in the same decoupled fire, before the render.
