...
* **`orchestrator/src/io/chatSessions.ts`**: `forkChat()`'s location-resurrection clone carries `seed`, `image_url`, `image_generated_at`, `image_rendered_input`, and `image_render_hash` forward onto the cloned row (no fresh render on fork), and now resurrects every copied active swipe's locations (not just the fork point's) plus re-keys the `location_swipe_images` associations onto the branch's own chat/swipe/location ids — so all pre-fork bgs transfer and cycle-back reuses recorded URLs (segway.md §2.7).
