-- orchestrator_settings.portrait_slot_bootstrap_prompt — the Portrait Studio slot bootstrapper's
-- prompt template (orchestrator/src/orchestrator/describeStudioSlots.ts): the LLM call that turns
-- a bare entity (name + whatever standing instructions it already has) into a starting set of
-- structured slot values for its own layer, fired on entity creation whenever no slots were
-- explicitly supplied. Without this, every entity was created with slots: {} and stayed that way
-- forever — the mutation loop (evoprompt.ts) only ever reuses slot names that already exist for a
-- layer, so a never-bootstrapped entity's chromosome (and therefore its composed image prompt)
-- stayed content-free no matter how many rounds ran. Empty override = the built-in default in
-- describeStudioSlots.ts (bi_principles.md §17), the same shape as every other prompt key.
--
-- orchestrator_settings.key's CHECK is rebuilt wholesale every time a key is added (the 0091/0092/
-- 0095/0105/0108/0111/0112 precedent) — the list below is the *complete* current vocabulary (every
-- key 0010-0112 added), not just the diff, so a fresh volume must land on the same constraint the
-- live DB has. Live-DB state is the source of truth; verified against pg_get_constraintdef before
-- writing this file. Idempotent hand-apply one-shot:
--   docker exec -i bigimagine-postgres psql -U bigimagine_admin -d bigimagine < db/migrations/0113_portrait_slot_bootstrap_prompt.sql
