-- orchestrator_settings.portrait_subject_describer_prompt — the Portrait Studio subject
-- describer's prompt template (docs/plans/portrait-studio-standalone-subjects-plan.md Part B):
-- the one synchronous LLM call that turns a bare subject name (+ optional seed) into the
-- entity's standing_instructions when an operator creates a Studio-native subject without
-- hand-typing full instructions. Empty override = the built-in default in
-- orchestrator/describeStudioSubject.ts (bi_principles.md §17), the same shape as every other
-- prompt key. Read live on each create; editable from the Settings tab's Portrait Subject
-- describer fieldset.
--
-- orchestrator_settings.key's CHECK is rebuilt wholesale every time a key is added (the 0091/0092/
-- 0095/0105/0108 precedent) — the list below is the *complete* current vocabulary (every key
-- 0010-0110 added), not just the diff, so a fresh volume must land on the same constraint the live
...
--   docker exec -i bigimagine-postgres psql -U bigimagine_admin -d bigimagine < db/migrations/0111_portrait_subject_describer_prompt.sql
