-- orchestrator_settings.visual_portraits_enabled — the household kill switch for the whole
-- Portrait Studio chain (routes under /v1/portraits/* except layers, the Portraits tab, and the
-- ActivePortrait box; docs/plans/portrait-chain-hardening-plan.md). Text 'true'/'false', default
-- 'true' when unset (the feature predates the switch and is already in use — an opt-out safety
-- valve, not an opt-in gate, unlike notifications_enabled's default-off).
--
-- orchestrator_settings.key's CHECK is rebuilt wholesale every time a key is added (the 0091/0092/
-- 0095/0105 precedent) — the list below is the *complete* current vocabulary (every key 0010-0107
-- added), not just the diff, so a fresh volume must land on the same constraint the live DB has.
-- Live-DB state is the source of truth; verified against pg_get_constraintdef before writing this
-- file. Idempotent hand-apply one-shot:
--   docker exec -i bigimagine-postgres psql -U bigimagine_admin -d bigimagine < db/migrations/0108_orchestrator_settings_visual_portraits_enabled.sql
