...
-- orchestrator_settings for the Ntfy driver. Applied by hand, same as 0008/0010/0016:
...
-- with no Cloudflare Access gate and anonymous read-write auth, so the topic name is the only
-- thing standing between "your phone" and "anyone's phone" — exactly the "possessing it grants
-- access on its own" test §12 asks. provider_credentials, encrypted, write-only, same shape as
-- brave_api_key.
--
-- ntfy_server_url is not a secret — it's the URL the orchestrator itself posts to, a selector
-- rather than a capability. Deliberately the internal
