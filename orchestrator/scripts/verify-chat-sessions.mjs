           ...
           // substring, so a text match alone can't tell the two inserts apart.
            if (sql.includes('insert into chat_sessions') && params.length === 12) {
             ...
             // (user_id, title, folder_id, params, tool_names, parent_chat_id, fork_message_id, kind,
              //  card_id, prompt_stack_preset_id, cleanup_preset_id, cleanup_enabled_at)
              const [userId, title, folderId, paramsJson, toolNames, parentChatId, forkMessageId, kind, cardId, promptStackPresetId, cleanupPresetId, cleanupEnabledAt] =
               params;
             const row = {
               chat_id: randomUUID(),
               user_id: userId,
