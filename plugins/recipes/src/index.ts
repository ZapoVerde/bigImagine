 ...
 * every other Notion-touching tool in this codebase). The read/write tools (get_recipes,
 * get_recipe, create_recipe, update_recipe, add_meal_plan_entry, get_meal_plan) only need
 * ctx.db/ctx.userId, supplied per-call, so they take no constructor deps at all.
 *
 * @api-declaration
