 ...
 * deps.llm (the LLM-extraction fallback path); generate_shopping_list_from_meal_plan needs
 * deps.notion (best-effort outbound sync of the list items it creates — undefined when Notion
 * isn't configured, same as every other Notion-touching tool in this codebase). The read/plan
 * tools (get_recipes, get_recipe, add_meal_plan_entry, get_meal_plan) only need ctx.db/ctx.userId,
 * supplied per-call, so they take no constructor deps at all.
 *
 * @api-declaration
...
export async function registerTools(deps: PluginDeps): Promise<RegisteredTool[]> {
  return [
    ...
    createGenerateShoppingListFromMealPlanTool(deps.notion),
  ];
}
